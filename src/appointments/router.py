...
def create_appointment(appointment: schemas.AppointmentCreate, db: Session = Depends(get_db)):
    appointment = service.get_by_barber_id_on_datetime(db=db, barber_id=appointment.barber_id,
...
def get_appointments(skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
    appointments = service.get(db=db, skip=skip, limit=limit)
    return appointments
...
def get_appointments_by_barber_id(barber_id: int, db: Session = Depends(get_db)):
    return service.get_by_barber_id(db=db, barber_id=barber_id)

@router.get("/{barber_id}/{appointment_date}", status_code=status.HTTP_200_OK, response_model=list[schemas.Appointment])
def get_appointments_by_barber_id_on_date(barber_id: int, appointment_date: date, db: Session = Depends(get_db)):

    return service.get_by_barber_id_on_date(db=db, barber_id=barber_id, appointment_date=appointment_date)

@router.get("/{barber_id}/{appointment_date}/{appointment_time}", status_code=status.HTTP_200_OK, response_model=schemas.Appointment)
def get_appointment_by_barber_id_on_datetime(barber_id: int, appointment_date: date, appointment_time: time, db: Session = Depends(get_db)):
